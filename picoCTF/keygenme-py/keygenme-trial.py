...
key_part_dynamic1_trial = "75fc1081"
...
def check_key(key, username_trial):
    ...
    if len(key) != len(key_full_template_trial):
        print(f"Wrong length {len(key)} {len(key_full_template_trial)}")
        return False
    else:
        ...
        for c in key_part_static1_trial:
            if key[i] != c:
                print(f"Wrong letter: {key[i]} should be: {c}")
                return False
        ...
        if key[i] != hashlib.sha256(username_trial).hexdigest()[4]:
            key_new_dynamical.append(hashlib.sha256(username_trial).hexdigest()[4])
            # return False
        else:
            i += 1

        if key[i] != hashlib.sha256(username_trial).hexdigest()[5]:
            key_new_dynamical.append(hashlib.sha256(username_trial).hexdigest()[5])
            # return False
        else:
            i += 1

        if key[i] != hashlib.sha256(username_trial).hexdigest()[3]:
            key_new_dynamical.append(hashlib.sha256(username_trial).hexdigest()[3])
            # return False
        else:
            i += 1

        if key[i] != hashlib.sha256(username_trial).hexdigest()[6]:
            key_new_dynamical.append(hashlib.sha256(username_trial).hexdigest()[6])
            # return False
        else:
            i += 1

        if key[i] != hashlib.sha256(username_trial).hexdigest()[2]:
            key_new_dynamical.append(hashlib.sha256(username_trial).hexdigest()[2])
            # return False
        else:
            i += 1

        if key[i] != hashlib.sha256(username_trial).hexdigest()[7]:
            key_new_dynamical.append(hashlib.sha256(username_trial).hexdigest()[7])
            # return False
        else:
            i += 1

        if key[i] != hashlib.sha256(username_trial).hexdigest()[1]:
            key_new_dynamical.append(hashlib.sha256(username_trial).hexdigest()[1])
            # return False
        else:
            i += 1

        if key[i] != hashlib.sha256(username_trial).hexdigest()[8]:
            key_new_dynamical.append(hashlib.sha256(username_trial).hexdigest()[8])
            # return False


        key_new_dynamical = "".join(key_new_dynamical)
        print(key_new_dynamical)
        return True

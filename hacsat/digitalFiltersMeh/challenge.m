...
%% 
% Setup Filters
%P = diag([1, 0.01, 0]) * 1e-5; % covariance matric
P = diag([1000, 1000, 1000]) * 1e-5; % covariance matric
Q = diag([1e-7, 1e-7, 0.0])^2; % noise matrix (measurement or model)
R = diag([1e-1, 1e-3, 1e-3])^2 ; % noise matrix (measurement or model)
...
for i = 1:steps
    ...
    [v,a] = q2rot(quat_diff(actual.q_att, target.q_att)) % transform quaternion to vector and angle
    temp = v(2)*a
    if abs(v(2)*a) >  (pi/8)
    ...
    % Get Observations
    q_att = startracker(actual); % get new observation
    disp("Error")
